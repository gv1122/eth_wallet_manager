...
export const ViewActivity: Button = {
    name: 'activity',
    ...
    run: async (client: Client, interaction: ButtonInteraction | CommandInteraction, walletAddress: string) => {
        ...
        if (txs.length === 0) {
            const embed = {
                title: `Activity`,
                description: `No transactions found for wallet: \`${walletAddress}\``,
                color: 16711680,
                image: {
                    url: BANNER_URL
                }
            ...
            for (let transaction of txs) {
                ...
                if (page > 7) {
                    ...
                    current += `🕛 ${myDatetimeString}\nExecuted a ${transaction.actionName} on ${transaction.chainName}\n\n`;
                    page++;
                }
            }
            ...
            const embed = {
                title: `📒 Transaction History for Wallet`,
